...
//Parameters
const double aspectRatio = 5.0; //gives relation between maxL and D, for rod-shaped particles: LD > 1
const double ki = 0.05 * tau; //internal spring constant, must be smaller dan dzeta*D*dt/2 otherwise system will explode, good value is 0.25
const double ko = 0.1 * tau; //overlap spring constant
const int relaxTime = 10; //set time to let system relax after growth step
const double growthRate = 0.00003 * (0.25 * (aspectRatio + diameter)) * tau * relaxTime; //number gives growth rate per timestep. experiments: 1.23 per hour, must be small compared to ki for relaxation
const double growthRateDev = 0.1 * growthRate; //sets deviation in growth rate
const double maxLengthDev = 0.1 * diameter * aspectRatio; //sets deviation in max length
...
const int Nmax = 5000; //maximum amount of particles
	...
	//Structure of particles with its properties
{
	...
	int ID;
	double D, dia, L, len, mu, Lmax; //size properties: diameter, actual diameter, restlength, actual length, growth rate, max length
...
void divide(Particle &pOld, Particle &pNew)
{
	...
	pNew.mu += randomMu(); //change growthrate of daughter cell
...
double thetadif(double theta1, double theta2)
{
...
void analyzeAll(vector<Particle> &p, int nop, Print &myPrint, int save, int ts, int &nod)
{
	...
	for (int d = 0; d < nop; d++) //third loop to find defects
	{
		...
		if (amonb > 0) //if using triangles than amonb > 1, if using neighbours than amonb > 0
		{
					...
					if (b > a)
					{
						...
						if (checknn == 1) //neighbours 1 and 2 are also neighbours of each other
						{
							...
							if ((theta12 < ta && theta23 < ta && theta31 > ta) || (theta23 < ta && theta31 < ta && theta12 > ta) || (theta12 < ta && theta31 < ta && theta23 > ta))
							{
								...
								int fp, sp, tp; double atm1, atm2, atm3; double thetafs, thetast, thetatf;
								double xmean = (p[d].xcpos + p[nb1].xcpos + p[nb2].xcpos) / 3; double ymean = (p[d].ycpos + p[nb1].ycpos + p[nb2].ycpos) / 3;
								atm1 = atan2((p[d].xcpos - xmean), (p[d].ycpos - ymean)) + pi;
								atm2 = atan2((p[nb1].xcpos - xmean), (p[nb1].ycpos - ymean)) + pi;
								atm3 = atan2((p[nb2].xcpos - xmean), (p[nb2].ycpos - ymean)) + pi;
								double smallA = min(min(atm1, atm2), atm3); double largeA = max(max(atm1, atm2), atm3);
								...
								thetafs = thetadif(p[fp].theta, p[sp].theta);
								thetast = thetadif(p[sp].theta, p[tp].theta);
								thetatf = thetadif(p[tp].theta, p[fp].theta);
								if ((thetafs + thetast + thetatf) > 0)
									p[d].ram += 1; //minus defect
								else if ((thetafs + thetast + thetatf) < 0)
									p[d].ram += 100; //plus defect
								else
									p[d].ram += 10; 
							}
							//notr++;
			...
			p[d].oop = sumsop / amonb; //use for sop nb, range is [-1,1]
			//if (notr > 0)
			...
			if (p[d].ram > 99)
				...
				p[d].colour = (int)((1 - p[d].oop) * 70 + 180);
		...
		if (ts % save == 0)
			myPrint.print_data(ts, p[d].ID, p[d].xcpos, p[d].ycpos, p[d].theta, p[d].len, p[d].dia, p[d].oop, p[d].ram); //save densities, in cluster 9 inputs are needed
	}
}
...
int main()
{
	...
	int nod; //number of defects
	int ts; //timestep
	int save = 5000; //determines how many times data is saved, for nop = 3000 time runs till 650000. 300 gives nice set of data

	string folderName = "nodef_Ld_5"; //name of data folder
	int startRound = 0;
	...
	{
		cout << "Type of sim: loop nr = " << round << ", nop = " << Nmax << ", L/D = " << aspectRatio << endl;
		cout << "Mu = " << growthRate / relaxTime << ", muDev = " << growthRateDev << ", Ldev = " << maxLengthDev << ", k_i = " << ki / tau << ", k_o = " << ko / tau << ", ON = " << orientNoise << endl;
		cout << "DF = plusMinRAM" << endl;
		...
		{			
			...
			if (ts % save == 0)
				analyzeAll(p, nop, print, save, ts, nod); //analyze system to find order parameter and defects

			if ((round == startRound) && (ts % (save * 2) == 0))
