...
void growAll(vector<Particle> &p, int ts){
    ...
    for(int i = 0; i < lengthBefore; i++){ //Unable to be range based since the ranging object changes length, leading to an infinite loop
        ...
        if(p[i].L > p[i].Lmax){
            ...
            p.front().str();
            p.back().str();
            TwoVec l1 = p.front().positions[1] - p.front().positions[0];
            TwoVec l2 = p.front().positions[2] - p.front().positions[1];
            std::cout << "Internal angle pOld " << angleBetweenVectors(l1, l2) << std::endl;
            TwoVec m1 = p.back().positions[1] - p.back().positions[0];
            TwoVec m2 = p.back().positions[2] - p.back().positions[1];
            std::cout << "Internal angle pOld " << angleBetweenVectors(l1, l2) << std::endl;
...
void run(std::string path){
    ...
    do{
        ...
        if(ts % writeTime == 0 && p.size() > 1) writeAll(p, outStream, ts);
        moveAll(p);
        ts++;
    } while(p.size() < Nmax);
}
